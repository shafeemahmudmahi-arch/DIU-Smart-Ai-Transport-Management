...
def generate_students():
    ...
    first_names = ["Arafat", "Nusrat", "Tanvir", "Farhana", "Rakib", "Tasnim", "Kamal", "Sadiya", "Rifat", "Jannatul"]
    last_names = ["Rahman", "Islam", "Ahmed", "Khan", "Hossain", "Ali", "Uddin", "Akter", "Khatun", "Begum"]
...
def login_screen():
    ...
    with col2:
        ...
        st.info("💡 Demo Student IDs (Use any of these with password 'diu123'):")
        st.write(st.session_state.students_db[['Student_ID', 'Name', 'Route']].head(3))

        role = st.selectbox("Select Role", ["Student", "Admin", "Driver"], key="role_select")
        
        if role == "Student":
            st.info("📝 Demo: Use a Student ID from above, Password: `diu123`")
...
def student_dashboard():
    ...
    with col2:
        st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
        avg_occupancy = route_buses['Current_Occupancy'].mean() if not route_buses.empty else 0
        st.metric("👥 Avg Occupancy", f"{avg_occupancy:.0f}/40")
        st.markdown('</div>', unsafe_allow_html=True)
    with col3:
        st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
        available_seats = (route_buses['Total_Capacity'] - route_buses['Current_Occupancy']).sum() if not route_buses.empty else 0
    ...
    with tab1:
        ...
        if not route_buses.empty:
            ...
            tooltip = {
                "html": "<b>Bus:</b> {Bus_ID}<br/><b>Speed:</b> {Speed} km/h<br/><b>Available Seats:</b> {Available_Seats}<br/><b>ETA:</b> {ETA}",
                "style": {"backgroundColor": "rgba(10, 22, 40, 0.9)", "color": "white", "border": "1px solid #39b54a"}
            }
            
            st.pydeck_chart(pdk.Deck(layers=[layer, text_layer], initial_view_state=view_state, tooltip=tooltip, map_style=None))
    ...
    with tab2:
        ...
        if not route_buses.empty:
            ...
            with col1:
                selected_bus = st.selectbox("Select Bus", route_buses['Bus_ID'].tolist(), key="seat_lock_bus")
            with col2:
    ...
    with st.popover("🤖 AI Assistant", use_container_width=False):
        ...
        user_question = st.text_input("Your question:", placeholder="When is the next bus?")
        if st.button("Send", use_container_width=True):
            if user_question:
                ...
                if "next bus" in user_question.lower() or "eta" in user_question.lower():
                    response = f"🚌 The next bus on your route will arrive in approx {random.randint(5, 15)} minutes."
                elif "seat" in user_question.lower():
                    seats = int(route_buses['Available_Seats'].sum()) if not route_buses.empty else 0
                    response = f"💺 Currently {seats} seats are available on your route."
                else:
                    response = "I can help you with: Bus schedules, ETAs, Seat availability, and Route information."
...
def admin_dashboard():
    ...
    with tab1:
        ...
        if not buses_db.empty:
            ...
            def get_color(occupancy):
                if occupancy < 20: return [57, 181, 74, 200]
                elif occupancy < 30: return [255, 193, 7, 200]
                else: return [220, 53, 69, 200]
                
            buses_db['color'] = buses_db['Current_Occupancy'].apply(get_color)
            ...
            tooltip = {
                "html": "<b>Bus ID:</b> {Bus_ID}<br/><b>Route:</b> {Route}<br/><b>Driver:</b> {Driver_Name}<br/><b>Occupancy:</b> {Current_Occupancy}/{Total_Capacity}<br/><b>Speed:</b> {Speed} km/h<br/><b>Status:</b> {Status}",
                "style": {"backgroundColor": "rgba(10, 22, 40, 0.95)", "color": "white", "border": "1px solid #39b54a"}
            }
    ...
    with tab2:
        ...
        with col1:
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(name='Current Buses', x=prediction_df['Route'], y=prediction_df['Current_Buses'], marker_color='#39b54a'))
            fig.add_trace(go.Bar(name='Required Buses', x=prediction_df['Route'], y=prediction_df['Required_Buses'], marker_color='#5dd9e8'))
            fig.update_layout(barmode='group', plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color='white', height=400)
            st.plotly_chart(fig, use_container_width=True)
            
        with col2:
            st.markdown("#### Predicted Peak Load")
            fig2 = go.Figure()
            fig2.add_trace(go.Scatter(x=prediction_df['Route'], y=prediction_df['Predicted_Peak_Load'], mode='lines+markers', line=dict(color='#39b54a', width=3), marker=dict(size=10)))
            fig2.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color='white', height=400, yaxis_title="Students")
    ...
    with tab3:
        ...
        revenue_data = [{'Route': r, 'Trips': random.randint(80, 150), 'Revenue': random.randint(5000, 12000), 'Compliance': random.randint(90, 100)} for r in routes]
        revenue_df = pd.DataFrame(revenue_data)
        fig = px.bar(revenue_df, x='Route', y='Revenue', color='Compliance', color_continuous_scale='Greens', title='Daily Revenue by Route')
    ...
    with tab4:
        ...
        col1, col2 = st.columns(2)
        with col1:
            report_type = st.selectbox("Report Type", ["Daily Operations", "Fleet Performance", "Revenue Analysis"])
            if st.button("📄 Generate Report", use_container_width=True):
                st.success("✅ Report generated successfully!")
        with col2:
            st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
            st.markdown(f"**Total Routes:** {len(routes)}<br>**Total Buses:** {len(buses_db)}<br>**System Uptime:** 99.8%", unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
...
def driver_dashboard():
    ...
    with col1:
        st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
        st.metric("👥 Current Load", f"{random.randint(15, 35)}/40")
        st.markdown('</div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with col1:
            student_id_input = st.text_input("Enter Student ID", placeholder="DIU2023XXXXXXX", key="scanner_input")
        with col2:
            st.markdown("<br>", unsafe_allow_html=True)
        ...
        if scan_button and student_id_input:
            ...
            if not student.empty:
                ...
                if student_data['Route'] == user_data['Route']:
                    st.markdown('<div class="scanner-verified">✅ VERIFIED</div>', unsafe_allow_html=True)
                    st.success(f"✅ Authorized: {student_data['Name']} ({student_data['Department']})")
                else:
                    st.markdown('<div class="scanner-denied">❌ DENIED</div>', unsafe_allow_html=True)
                    st.error(f"⚠️ Wrong Route! Student route: {student_data['Route']}")
            else:
                st.markdown('<div class="scanner-denied">❌ NOT FOUND</div>', unsafe_allow_html=True)
